...
files = dir('C:\Users\Andrew McNally\Downloads\ROB599_PerceptionProject_SampleCode/deploy/*/*/*_image.jpg');
numel(files)
...
for k = 1:size(bbox, 1)
    ...
    if(b(11) == 0)
        ...
        maxY = max(vert_2D(2,:));
        minY = min(vert_2D(2,:));
        vert_square = [[maxX; maxY], [maxX; minY], [minX; maxY], [minX; minY]];
        es = [1, 2, 4, 3;
               2, 4, 3, 1];
        %square_edges = 
        ...
        for i = 1:size(es, 2)
            ...
            figure(1)
            plot(vert_square(1, e), vert_square(2, e), 'color', clr)
